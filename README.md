...
The online multiplayer screen is available from the mode-select menu in every build. **Unity Relay** is the primary path: managed join codes, NAT traversal, DTLS encryption, IP concealment, and migration coordination. It can be disabled instantly for all players from the Unity Dashboard (Remote Config → `multiplayer_enabled`) without a new build, via `MultiplayerKillSwitch`. **Direct IP** host/join, which works without Unity Cloud over a configurable UDP port (LAN/VPN recommended; public hosts may need UDP port forwarding), only appears in the UI as a fallback while Relay is disabled that way. Both paths exercise authenticated seats, readiness, host-only authority start, clean leave, and live RTT measurement.
