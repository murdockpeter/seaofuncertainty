...
> Archived on **2026-09-09** after 252 of 299 tracked items were completed. The remaining 47 items moved to the active root `TODO.md`; this file preserves the milestone history and repository evidence.
...
- [x] Add scalable text controls.
- [x] Complete keyboard-only navigation through every modal and map workflow.
- [x] Re-enable controller navigation with calibrated dead zones and one-step-per-input focus behavior.
- [x] Add subtitles/text descriptions for every audio event.
- [x] Add separate effects, music/ambient, and master volume controls.
...
- [x] Replace prototype formation counters with code-owned production meshes, materials, markings, wakes, and licensed provenance after formally reopening art direction.
- [x] Create a formal production art-direction guide and asset approval workflow.
- [x] Complete the asset provenance/licensing ledger for every distributable asset.
- [x] Replace provisional generated art before public release where licensing or quality requires it.
- [x] Add production-quality action/rating icons and Contact/status/card visuals.
...
- [x] Add AI decision-distribution telemetry by action, formation type, phase, and situation.
- [x] Add tactical AI fixtures for prosecution, objective movement, entropy recovery, weapon commitment, reactions, cards, and disengagement.
- [x] Add baseline-versus-candidate simulations for measurable AI improvement.
- [x] Add strategic variety while preserving reproducibility for a fixed seed/profile.
...
- [x] Define distinct surface-formation behavior through maneuver, escort, control, Screen/Support, naval logistics, and role-specific sensor/weapon envelopes.
- [x] Define a bounded submarine Contact/combat procedure using deep/shallow Signature state, public Contact domain, ASW Search ratings, and an Identified-datum requirement for Standard/Heavy attacks.
- [x] Decide whether air groups remain formations or become mission packages. **Decision:** retain Formation state and treat each activation as an abstract mission package rather than continuously tracked aircraft.
- [x] Add carrier and air-operation behavior: force-projection/HQ carriers, mission-radius air activations, terrain abstraction, distinct logistics, scoring, Search, Strike, and Support roles.
- [x] Add nuanced Search and Signature profiles by formation kind, declared mode, target Signature, range, movement posture, Loud state, and Support.
- [x] Add salvo-dependent outer, area, point, and EW missile-defense layers.
- [x] Add finite Light/Standard/Heavy weapon inventories, expenditure on every commitment, and logistics-only reload restrictions.
- [x] Add printed electronic-warfare and cyber ratings that affect Signature, layered defense, and private Search resolution.
- [x] Add mobile logistics formations plus typed port, anchorage, and airfield repair access.
- [x] Add authoritative weather severity affecting Search, air operations, and naval High Tempo movement, including scheduled changes.
- [x] Add Centralized, Mission Command, and Distributed command architectures with scenario-defined attention capacity.
- [x] Move scenario metadata and complete formation definitions into the editable external `Resources/Data/scenario-catalog.json` authoring format.
- [x] Add a Unity Scenario Catalog Editor with validation and formation/rating/magazine editing.
- [x] Add deterministic scheduled weather, command, and reinforcement events to the operational timeline with save/load support.
- [x] Define how connected theaters handle off-map movement and transitions in gameplay through Exit/Reinforcement regions, deterministic transfer records, delayed arrivals, blocked-entry retries, and information-safe state preservation.
...
- [x] Define a versioned authoritative command/event format using stable scenario, formation, Contact, card, synchronized-event, and hex IDs plus explicit player choices.
- [x] Add deterministic replay from recorded commands and the random seed with per-command state digests, JSON round-trip, ownership validation, and divergence diagnostics.

### Print-and-play

- [x] Generate formation cards from the same data used by Unity.
- [x] Generate formation counters and Contact markers.
- [x] Generate Ready-Time, Command, Endurance, weapon, and entropy tracks.
- [x] Generate the 36-card Entropy and 24-card Response decks from structured data.
- [x] Generate scenario setup sheets and player aids from structured rules data.
...
## Post-archive completions

- [x] Split the large editor smoke entry point into independently runnable unit, integration, presentation, and build-validation suites.
- [x] Add callback-level automated UI interaction coverage for Command Response and Strike targeting, Hotseat handoff privacy, persisted settings, fullscreen transition policy, and modal navigation.
- [x] Synchronize multiplayer Phase 1 through versioned authoritative command envelopes and side-scoped snapshots rather than UI objects.
- [x] Validate authority-side hidden information with owned-formation filtering and opaque Contact references that never expose enemy formation IDs.
- [x] Add sequence/digest desynchronization detection, authoritative snapshot recovery, deterministic replay evidence, and server-private dispute diagnostics.
- [x] Select Unity Multiplayer Services Sessions with DTLS Relay and direct Unity Transport command payloads as the production listen-server architecture.
- [x] Add two-seat lobby setup, invite/deep-link codes, ready locking, authenticated reconnection, Unity service integration, and signed interrupted-match recovery.
